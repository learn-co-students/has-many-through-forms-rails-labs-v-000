...
describe 'Posts', type: 'feature' do
  ...
  describe 'show' do
    before do
      ...
      @post = Post.create(title: 'So Awesome',
                          content: 'Man am I a great blogger OR WHAT!')
      @post.categories << @category
      @user = User.create(username: 'RealBigFish')
      @comment1 = Comment.create(content: 'This was great!',
                                 user: @user,
                                 post: @post)
    end
    ...
    it 'should display a list of unique users who have commented on the post' do |_variable|
      @comment2 = Comment.create(
        content: "And another thing, how come there aren't any vegetabls at the movie theater...",
        user: @user,
        post: @post
      )
      visit post_path(@post)
